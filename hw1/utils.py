...
def generate_batched_data(data, label, batch_size=32, shuffle=False, seed=None):
    ...
    for m in range(0, len2, batch_size):
      batched_data.append(data[m:m + batch_size])

    for n in range(0, len2, batch_size):
      batched_label.append(label[n:n + batch_size])
...
def plot_curves(train_loss_history, train_acc_history, valid_loss_history, valid_acc_history):
    ...
    #############################################################################
    e = np.arange(len(train_loss_history))
    plt.plot(e, train_loss_history, 'g', label = "training")
    plt.plot(e, valid_loss_history, 'r', label = "validation")
    ...
    p = np.arange(len(train_loss_history))
    plt.plot(p, train_acc_history, 'g', label = "training")
    plt.plot(p, valid_acc_history, 'r', label = "validation")
