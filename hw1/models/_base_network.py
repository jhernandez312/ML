...
class _baseNetwork:
    ...
    def softmax(self, scores):
        ...
        numerator = np.exp(z)
        denomenator = np.sum(numerator, axis=1, keepdims=True)
    ...
    def cross_entropy_loss(self, x_pred, y):
        ...
        #############################################################################
        num_row = x_pred.shape[0]
        #x_pred is already ran through softmax so we can directly use it
        cross_entropy = -np.log(x_pred[range(num_row), y])
        loss = np.sum(cross_entropy)/num_row
    ...
    def compute_accuracy(self, x_pred, y):
        ...
        #############################################################################
        prob = self.softmax(x_pred)
        predicted = np.argmax(prob, axis=1)
        acc = np.mean(y == predicted)
        #############################################################################
        #                              END OF YOUR CODE                             #
        #############################################################################
        return acc
