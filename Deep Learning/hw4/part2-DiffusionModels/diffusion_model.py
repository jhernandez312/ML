...
class DiffusionModel:
    ...
    def compute_loss_on_batch(self, data, cond, seed = None):
        ...
        ** Use DL_random() to generate any random numbers **
        ...
        #######################################################################
        is_unconditional = DL_random(shape=(1,), normal=False, seed=seed).item() < self.p_uncond
        if is_unconditional:
        ...
        ########################################
        timesteps = DL_random(shape=(data.size(0),), int_range=(0, self.denoising_steps), normal=False, seed=seed).to(self.device)
        noise = DL_random(data.shape, normal=True, seed=seed).to(self.device)

        noisy_data = self.noise_scheduler.add_noise(data, noise, timesteps)
        predicted_noise = self.noise_pred_net(noisy_data, timesteps, cond)
    ...
    def generate_sample(self, cond, guidance_weight = 0.0, threshold = False, seed = None):
        ...
        ** Use DL_random() to generate any random numbers **
        ...
        with torch.no_grad():
            ...
            ###################################################################################
          sample_shape = (cond.size(0),) + self.input_shape
          sample = DL_random((cond.shape[0],) + self.input_shape, normal = True, seed = seed).to(self.device) 
          
          for t in reversed(range(self.denoising_steps)):
            t_tensor = torch.full((cond.size(0),), t, dtype=torch.long, device=self.device)

            cond_pred_noise = noise_pred_net(sample, t_tensor, cond)

            uncond_pred_noise = noise_pred_net(sample, t_tensor, torch.zeros_like(cond))

            guided_noise = ((1 + guidance_weight) * cond_pred_noise) - (guidance_weight * uncond_pred_noise)
            
            sample = self.noise_scheduler.denoise_step(guided_noise, t, sample, threshold, seed)
            ####################################################################################
            #                                 END OF YOUR CODE                                 #
            ####################################################################################
